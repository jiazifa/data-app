...
import { CreateUserOptions, createUser, isUserExists, query_user_by_options, useUserList } from "@/service/user";
...
const UserListPage = () => {
    ...
    const onAddUserAction = async (values: CreateUserOptions) => {
        try {
            const u = await isUserExists({ name: values.name });
            if (u) {
                throw new Error(`用户已存在：${JSON.stringify(values)}`)
            }
    ...
    const onDeleteUserAction = (user: User) => {
        modals.openConfirmModal({
            ...
            onConfirm: () => {
                console.log(`删除用户：${user.user_name} 暂时不可用`);
            },
